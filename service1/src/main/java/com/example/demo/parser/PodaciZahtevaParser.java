...
public class PodaciZahtevaParser implements Parser<PodaciZahteva> {
	...
	@Override
	public Element parse(String namespace, PodaciZahteva type, Document document) {
		Element podaciZahteva = document.createElement(namespace + ":Podaci_zahteva");
		Element trazenaInformacija = document.createElement(namespace + ":Trazena_informacija");
		trazenaInformacija.setTextContent(type.getTrazenaInformacija());
		Element tipDostave = document.createElement(namespace + ":Tip_dostave");
		tipDostave.setTextContent(type.getTipDostave() + "");
		Element opisDostave = document.createElement(namespace + ":Opis_dostave");
