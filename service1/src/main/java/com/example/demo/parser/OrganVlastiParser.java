...
public class OrganVlastiParser implements Parser<OrganVlasti> {
	...
	public OrganVlasti parse(Element element) {
		...
		Adresa adresa = this.adresaParser.parse((Element) element.getElementsByTagName("organ_vlasti:Adresa").item(0));
		return new OrganVlasti(naziv, adresa);
	}

	@Override
	public Element parse(String namespace, OrganVlasti type, Document document) {
		Element organVlasti = document.createElementNS(namespace, "Organ_vlasti");
		...
		Element naziv = document.createElement(namespace + ":Naziv");
