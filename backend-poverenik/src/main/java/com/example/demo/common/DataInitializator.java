...
public class DataInitializator {
	...
	public void dataInit() throws UnsupportedEncodingException, IOException {
		...
		this.fusekiManager.save(ResenjeRDF.RESENJE_GRAPH, model, Constants.RESENJE_SHAPE);

		String temp = soap.sendSOAPMessage(
				this.domParser.buildDocument(String.format("<pretraga><broj>%s</broj></pretraga>", 1)),
				SOAPDocument.zahtev_pdf);
